...
myApp.config([
  ...
  function($compileProvider) {
    $compileProvider.aHrefSanitizationWhitelist(
      /^\s*(https?|ftp|mailto|chrome-extension|chrome):/
    );
    $compileProvider.imgSrcSanitizationWhitelist(
      /^\s*(https?|local|data|chrome-extension|chrome):/
    );
  }
]);
myApp.controller("mainController", [
  ...
  function($scope, manager) {
    ...
    $scope.loadInstalledExtensions = function() {
      chrome.management.getAll(function(data) {
        var loadedExtensions = data.map(function(extensionItem) {
          ...
          extension.iconURL = extensionItem.hasOwnProperty("icons")
            ? manager.getIconUrl(extensionItem.icons)
            : "/assets/icons/no-extension-icon.png";
          return extension;
        });
    ...
    $scope.enableAllExtensions = function() {
      try {
        $scope.extensions.forEach(function(extension, index) {
          (function(currentIndex) {
            chrome.management.setEnabled(extension.id, true, function() {
              $scope.$apply(function() {
                $scope.extensions[currentIndex].enabled = true;
                $scope.disabledItems = $scope.extensions.filter(function(item) {
                  return !item.enabled;
                });
              });
            });
      ...
      try {
        chrome.management.setEnabled(extensionId, false, function() {
          $scope.$apply(function() {
            ...
            $scope.selectedExtension = $scope.extensions[index];
            $scope.disabledItems = $scope.extensions.filter(function(
              extension
            ) {
              return !extension.enabled;
            });
    ...
    $scope.disableAllExtensions = function() {
      try {
        $scope.extensions.forEach(function(extension, index) {
          if (extension.id !== chrome.runtime.id) {
            (function(currentIndex) {
              chrome.management.setEnabled(extension.id, false, function() {
                $scope.$apply(function() {
                  $scope.extensions[currentIndex].enabled = false;
                  $scope.disabledItems = $scope.extensions.filter(function(item) {
                    return !item.enabled;
                  });
                });
...
myApp.directive("extenionicon", function() {
  return {
    ...
    template:
      '<img ng-src={{url}} class="img-circle plugin-lg-icon pull-left" />'
  };
});
